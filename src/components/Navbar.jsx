...
export default function Navbar() {
  ...
  return (
    <nav className='bg-[#F0EFFB] px-10 py-3 checkpoint:px-12 checkpoint:py-[22px] flex items-center flex-wrap z-50'>
      ...
      <button onClick={handleMenu} className='checkpoint:hidden px-[24px] py-3 z-50'>
        {isMenuOpen ? (<AiOutlineClose className='text-2xl' />): (<img src="src/icons/menu.svg" alt="" />)}
      </button>

      {isMenuOpen && (
        <ul className='checkpoint:hidden text-lg flex flex-col gap-4 absolute inset-0 bg-[#F0EFFB] items-center justify-center'>
          <li>
